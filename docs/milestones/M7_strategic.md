...
- **Module:** `temper_ai/lifecycle/` (LifecycleAdapter, ProjectClassifier, ProfileRegistry)
...
- **CLI:** `temper-ailifecycle profiles|classify|preview|history|check`
...
- **Module:** `temper_ai/goals/` (GoalProposer, AnalysisOrchestrator, GoalSafetyPolicy, GoalReviewWorkflow)
...
- **CLI:** `temper-aigoals list|propose|review|approve|reject|status`
...
- **Module:** `temper_ai/portfolio/` (ResourceScheduler, ComponentAnalyzer, PortfolioOptimizer, KnowledgeGraph)
- **Key design:** WFQ scheduling, Jaccard similarity for component sharing, 4-metric scorecards
- **CLI:** `temper-aiportfolio list|show|run|scorecards|recommend|components|graph stats|graph query`
