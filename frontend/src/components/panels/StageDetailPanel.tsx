...
export function StageDetailPanel({ stageId }: StageDetailPanelProps) {
  ...
  if (!stage) {
    return (
      <div className="p-4 text-sm text-maf-text-muted">Stage not found.</div>
    );
  }
    ...
    <div className="flex flex-col gap-4 p-4">
      ...
      {iterations.length > 1 && (
        <div className="flex items-center gap-2">
          <span className="text-xs text-maf-text-muted">Iteration:</span>
      ...
      {executed > 0 && (
        <div className="flex flex-col gap-1">
          <span className="text-xs text-maf-text-muted">Agent Results</span>
          <div className="flex h-3 w-full overflow-hidden rounded-full bg-maf-panel">
            {successPct > 0 && (
              <div
                className="bg-maf-completed transition-all"
                style={{ width: `${successPct}%` }}
              />
            )}
            {failPct > 0 && (
              <div
                className="bg-maf-failed transition-all"
                style={{ width: `${failPct}%` }}
              />
            )}
          </div>
          <div className="flex gap-3 text-xs text-maf-text-dim">
            <span className="flex items-center gap-1">
              <span className="inline-block size-2 rounded-full bg-maf-completed" />
              {succeeded} succeeded
            </span>
            <span className="flex items-center gap-1">
              <span className="inline-block size-2 rounded-full bg-maf-failed" />
              {failed} failed
            </span>
          </div>
        </div>
            ...
            {stage.collaboration_events.map((evt, i) => (
              <div key={i} className="flex gap-3 relative">
                ...
                {i < stage.collaboration_events!.length - 1 && (
                  <div className="absolute left-[7px] top-5 bottom-0 w-px bg-maf-border" />
                )}
                {/* Dot */}
                <div className="w-3.5 h-3.5 rounded-full bg-maf-accent/30 border-2 border-maf-accent shrink-0 mt-0.5 z-10" />
                {/* Content */}
                <div className="flex-1 rounded-md bg-maf-panel p-2 text-xs text-maf-text mb-2">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="font-medium text-maf-accent">{evt.event_type}</span>
                    {evt.from_agent && <span className="text-maf-text-muted">from {evt.from_agent}</span>}
                    {evt.to_agent && <span className="text-maf-text-muted">to {evt.to_agent}</span>}
                    {evt.agents_involved && evt.agents_involved.length > 0 && (
                      <span className="text-maf-text-muted">({evt.agents_involved.join(', ')})</span>
                    )}
                  </div>
                  {evt.timestamp && (
                    <span className="text-[10px] text-maf-text-dim mt-0.5 block">{formatTimestamp(evt.timestamp)}</span>
                  )}
