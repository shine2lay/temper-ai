...
export function serializeWorkflowConfig(meta: WorkflowMeta, stages: DesignStage[]): Record<string, unknown> {
  ...
  // Only write what the author actually chose, the way the safety block
  // below already does. Writing all four unconditionally meant that
  // opening a workflow in Studio and saving it without a single edit
  // added an error_handling block it never had — pinning retries and
  // failure policy to today's defaults, and showing up as a spurious diff
  // in every workflow anyone happened to open.
  const errObj: Record<string, unknown> = {};
