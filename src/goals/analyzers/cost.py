...
class CostAnalyzer(BaseAnalyzer):
    ...
    def _query_data(self, lookback_hours: int) -> tuple:
        ...
        from src.storage.database.models import AgentExecution, LLMCall
...
def _analyze_agent_costs(agents: list) -> List[GoalProposal]:
    ...
    for a in agents:
        by_agent[a.agent_name] = by_agent.get(a.agent_name, 0) + (a.estimated_cost_usd or 0)
        ...
        if share > HIGH_COST_AGENT_SHARE:
            ...
            proposals.append(GoalProposal(
                ...
                expected_impacts=[ImpactEstimate(
                    metric_name="cost_usd", current_value=agent_cost,
                    expected_value=agent_cost - savings,
                    improvement_pct=SAVINGS_FACTOR * PCT_MULTIPLIER, confidence=MID_CONFIDENCE,
                )],
                evidence=GoalEvidence(
                    metrics={"agent_cost_usd": agent_cost, "total_cost_usd": total_cost, "cost_share": share},
    ...
    for model_name, stats in models[1:]:
        ...
        if ratio > MODEL_COST_RATIO:
            ...
            proposals.append(GoalProposal(
                ...
                risk_assessment=RiskAssessment(
                    level=GoalRiskLevel.MEDIUM, blast_radius=f"model:{model_name}", reversible=True,
                ),
                effort_estimate=EffortLevel.SMALL,
                expected_impacts=[ImpactEstimate(
                    metric_name="cost_usd", current_value=stats["cost"],
                    expected_value=stats["cost"] - savings,
                    improvement_pct=improvement, confidence=LOW_CONFIDENCE,
                )],
                evidence=GoalEvidence(
                    metrics={"cost_per_call": per_call, "cheap_cost_per_call": cheapest_per_call},
