...
Usage:
    maf autonomy status [--agent NAME]     Show autonomy levels
    maf autonomy escalate --agent NAME     Manual escalation
    maf autonomy deescalate --agent NAME   Manual de-escalation
    maf autonomy emergency-stop --reason   Activate emergency stop
    maf autonomy resume --reason           Deactivate emergency stop
    maf autonomy budget [--scope SCOPE]    Show budget status
    maf autonomy history [--agent NAME]    Transition audit log
"""
...
def emergency_stop(reason: str, db: str) -> None:
    ...
    console.print("Resume with: maf autonomy resume --reason <reason>")
