"""Server delegation for ``maf run``.

When a MAF server is running, ``maf run`` delegates workflow execution
...
def detect_server(
    ...
    """Check if a MAF server is running and return a client if so.
...
def delegate_to_server(
    ...
    """Delegate workflow execution to a running MAF server.
