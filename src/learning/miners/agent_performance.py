...
from src.learning.models import PATTERN_AGENT_PERFORMANCE, LearnedPattern
    ...
    def mine(self, lookback_hours: int = DEFAULT_LOOKBACK_HOURS) -> List[LearnedPattern]:
        ...
        for name, stats in agent_stats.items():
            if stats["total"] < MIN_EXECUTIONS:
                continue
            patterns.extend(
                _check_agent(name, stats, avg_duration)
            )
...
def _aggregate_agent_stats(executions: list) -> dict:
    ...
    for ex in executions:
        ...
        if name not in stats:
            stats[name] = {"total": 0, "success": 0, "durations": [], "workflow_ids": set()}
        stats[name]["total"] += 1
...
def _overall_avg_duration(executions: list) -> float:
    ...
    durations: list[float] = [e.duration_seconds for e in executions if e.duration_seconds]
    if not durations:
        return 0.0
    ...
    if stats["durations"] and avg_duration > 0:
        ...
        if agent_avg > avg_duration * SLOW_FACTOR:
            patterns.append(LearnedPattern(
                ...
                evidence={"agent_avg_duration": agent_avg, "overall_avg": avg_duration},
