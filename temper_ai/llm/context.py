...
``compress``
    ...
    ``context_status`` and, once the transcript passes a threshold, a note on
    the latest tool result naming the largest compressible ranges. It writes
    the summaries itself. The transcript is never mutated: a compressed range
    is hidden from the wire view, and the summary lives in the model's own
    ``compress`` call, which stays in place as the anchor — so what the model
    remembers is exactly what it chose to write down, and every summary is in
    the run's tool-call log for audit. If the model does not act and the hard
    limit is reached, tool results outside the working set are evicted with a
    visible stub (the model knows what is gone and can re-read it) — never cut
    from the head.
...
model wrote down ten turns ago still names the same message.
"""
...
class Block:
    id: str
    tier: int
    start: int  # first transcript index hidden by this block
    end: int  # last transcript index hidden by this block (inclusive)
    anchor_call_id: str  # tool_call_id of the compress call that created it
    summary: str
    tokens_before: int
    tokens_after: int
    children: list[str] = field(default_factory=list)
    anchor: int | None = None  # index of the compress tool result, once appended
    active: bool = True

    @property
    def extent_end(self) -> int:
        """Last transcript index that belongs to this block: its anchor once known."""
        return self.anchor if self.anchor is not None else self.end
...
class ContextCompressor:
    ...
    def prepare(self, messages: list[dict]) -> list[dict]:
        ...
        self._resolve_anchors(messages)
    ...
    def view(self, messages: list[dict]) -> list[dict]:
        """The message list the provider sees: hidden ranges removed, tags rendered."""
        ...
        for i, msg in enumerate(messages):
            ...
            if role == "tool":
                ...
                if i in self._evicted:
                    content = (
                        f"[evicted to stay under the context limit: {names.get(m.get('tool_call_id') or '', 'tool')} "
                        f"result, ~{_fmt(_message_tokens(msg))} tokens. Re-run the tool if you need it.]"
                    )
                m["content"] = (
        ...
        return [
            {
                ...
                "function": {
                    ...
                    "description": (
                        ...
                        "Rejected when a range touches the task message, the last user message, part of an existing "
                        "block, or when the summary is not smaller than what it replaces."
                    ),
                    "parameters": {
                        "type": "object",
                        "properties": {
    ...
    def compress(self, messages: list[dict], ranges: Any, *, tool_call_id: str) -> str:
        ...
        planned: list[tuple[int, int, str, list[str], list[Block], int, int]] = []
        for r in ranges:
            ...
            tokens_before = sum(
                _message_tokens(messages[i]) for i in range(s, e + 1) if i not in hidden
            )
            tokens_after = len(summary) // 3 + 4
            if tokens_after >= tokens_before:
                raise ContextError(
                    f"summary for {ref(s)}–{ref(e)} is ~{tokens_after} tokens but the range is only "
                    f"~{tokens_before}; nothing would be gained. Compress a larger range or write less."
                )
            planned.append(
                (s, e, summary, notes, children, tokens_before, tokens_after)
            )

        planned.sort(key=lambda p: p[0])
        for (s1, e1, *_), (s2, *_rest) in zip(planned, planned[1:], strict=False):
            if s2 <= e1:
                raise ContextError(
                    f"ranges overlap: {ref(s1)}–{ref(e1)} and one starting at {ref(s2)}"
                )

        before = estimate_messages_tokens(self.view(messages))
        lines = []
        reclaimed = 0
        for s, e, summary, notes, children, tokens_before, tokens_after in planned:
            block = Block(
                id=f"b{len(self.blocks) + 1}",
                tier=1 + max((c.tier for c in children), default=0),
                start=s,
                end=e,
                anchor_call_id=tool_call_id,
                summary=summary,
            ...
            line = f"{ref(s)}–{ref(e)} → {block.id} (tier {block.tier}, {tokens_before:,} → {tokens_after:,} tokens)"
            if children:
                line += f", folds {', '.join(block.children)}"
            if notes:
                line += f"; {'; '.join(notes)}"
            lines.append(line)
        after = max(before - reclaimed, 0)
        logger.info(
    ...
    def decompress(
        ...
        *,
        ...
        names = self._tool_names(messages)
        owner = self._owners(messages)
        child_by_start = {self.blocks[c].start: self.blocks[c] for c in block.children}
        # a child's anchor pair carries nothing its summary does not; skip it
        skip = {
            j
            for c in block.children
            if (a := self.blocks[c].anchor) is not None
            for j in (a, owner.get(a))
        }
        parts = [
    ...
    def _anchor_indices(self) -> set[int]:
        return {
            b.anchor for b in self.blocks.values() if b.active and b.anchor is not None
        }
    ...
    def _resolve(
        ...
        if m := _BLOCK.match(token):
            ...
            if block.anchor is None:
                raise ContextError(
                    f"{token} was created this turn; use it as a boundary from the next turn on"
                )
            return block.start if start else block.anchor
    ...
    def _children_for(self, s: int, e: int) -> list[Block]:
        """Active blocks that [s, e] folds whole (content and anchor). A block's
        content and its anchor are separated by whatever happened in between;
        that gap is live and may be compressed on its own. Taking the content
        without the anchor, or the anchor without the content, is an error."""
        children = []
        for b in self.blocks.values():
            if not b.active:
                continue
            if e < b.start or s > b.extent_end:
                continue  # apart
            if b.end < s and (b.anchor is None or e < b.anchor):
                continue  # in the gap between the block's content and its anchor
            if b.anchor is None:
                raise ContextError(
                    f"{b.id} was created this turn; use it as a boundary from the next turn on"
                )
            if s <= b.start and e >= b.anchor:
                children.append(b)
            elif s <= b.start:
                raise ContextError(
                    f"{ref(s)}–{ref(e)} contains {b.label()} but not its anchor {ref(b.anchor)}; "
                    f"end the range at {b.id} to fold it, or start it after {ref(b.end)}"
                )
            else:
                raise ContextError(
                    f"{ref(s)}–{ref(e)} would hide {ref(b.anchor)}, the anchor of {b.label()}, while its "
                    f"content stays hidden; start the range at {b.id} to fold it, or end it before {ref(b.anchor)}"
                )
        return children

    def _resolve_anchors(self, messages: list[dict]) -> None:
        pending = [b for b in self.blocks.values() if b.anchor is None]
        if not pending:
            return
        for i, m in enumerate(messages):
            if m.get("role") != "tool":
                continue
            for b in pending:
                if m.get("tool_call_id") == b.anchor_call_id and i > b.end:
                    b.anchor = i
    ...
    def _compressible_ranges(self, messages: list[dict]) -> list[tuple[int, int, int]]:
        ...
        for i in range(len(messages) + 1):
            ...
            if live:
                if start is None:
                    start, tokens = i, 0
                tokens += _message_tokens(messages[i]) if i not in self._evicted else 0
    ...
    def _append_nudge(self, view: list[dict], messages: list[dict], est: int) -> None:
        if (
            not view
            or view[-1].get("role") != "tool"
            or not isinstance(view[-1].get("content"), str)
        ):
            return
        notes = []
        ...
        if notes:
            view[-1] = dict(view[-1])
            view[-1]["content"] = f"{view[-1]['content']}\n\n[context] " + " ".join(
                notes
            )
