...
def _ensure_llm_data(data: LLMCallData | None, kwargs: dict[str, Any]) -> LLMCallData:
...
def _ensure_tool_data(
    data: ToolCallData | None, kwargs: dict[str, Any]
) -> ToolCallData:
...
class SQLObservabilityBackend(  # noqa: god
    ...
    def __init__(self, buffer: Any = None) -> None:
        ...
        self._buffer: ObservabilityBuffer | None
        if buffer is None:
    ...
    def track_workflow_start(
        ...
        **kwargs: Any,
        ...
        workflow_exec = WorkflowExecution(
            ...
            workflow_version=workflow_config.get("workflow", {}).get(
        ...
        with get_session() as session:
            ...
            if wf:
                ...
                wf.duration_seconds = safe_duration_seconds(
                    wf.start_time, end_time_utc, context=f"workflow {workflow_id}"
                )
    ...
    def track_stage_start(
        ...
        stage_exec = StageExecution(
            ...
            stage_version=stage_config.get("stage", {}).get(
                "version", _DEFAULT_VERSION
            ),
        ...
        with get_session() as session:
            ...
            if st:
                ...
                st.duration_seconds = safe_duration_seconds(
                    st.start_time, end_time_utc, context=f"stage {stage_id}"
                )
    ...
    def track_agent_start(
        ...
        agent_exec = AgentExecution(
            ...
            agent_version=agent_config.get("agent", {}).get(
                "version", _DEFAULT_VERSION
            ),
        ...
        with get_session() as session:
            ...
            if ag:
                ...
                ag.duration_seconds = safe_duration_seconds(
                    ag.start_time, end_time_utc, context=f"agent {agent_id}"
                )
    ...
    def get_agent_execution(self, agent_id: str) -> AgentExecution | None:
