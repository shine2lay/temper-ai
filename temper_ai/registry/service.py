...
from temper_ai.registry._schemas import AgentRegistryEntry, MessageRequest, MessageResponse
...
class AgentRegistryService:
    ...
    def register_agent(
        ...
        if not name:
            raise ValueError(f"Agent config at '{config_path}' must have a 'name' field")
    ...
    def list_agents(self, status: Optional[str] = None) -> List[AgentRegistryEntry]:
        """List registered agents, optionally filtered by status."""
        return self._store.list_all(status_filter=status)

    def get_agent(self, name: str) -> Optional[AgentRegistryEntry]:
    ...
    def _load_agent(self, entry: AgentRegistryEntry) -> Any:
        ...
        return StandardAgent(config=config_snapshot)
