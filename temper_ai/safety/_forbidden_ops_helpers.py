...
def _compile_pattern_category(
    prefix: str, category: str, pattern_dict: dict[str, dict[str, Any]]
...
def _stringify_if_not_none(value: Any) -> str | None:
    """Return str(value) if value is not None, else None."""
    return str(value) if value is not None else None


def _extract_bash_tool_args(args: Any) -> str | None:
    ...
    if isinstance(args, dict):
        return cast(str | None, args.get(COMMAND_KEY))
    if isinstance(args, str):
        return cast(str | None, args)
    return None


def extract_command(action: dict[str, Any]) -> str | None:
...
def get_remediation_hint(category: str) -> str:
    ...
    return hints.get(
        category, "Review operation for safety and use approved alternatives."
    )
