...
class CollaborationConfig(BaseModel):
    ...
    max_dialogue_rounds: int | None = Field(
        default=DEFAULT_MAX_RETRIES,
        gt=0,
        description="Maximum number of dialogue rounds",
    )
    round_budget_usd: float | None = Field(
        default=None, gt=0.0, description="Cost budget per dialogue session (USD)"
    )
    ...
    context_window_rounds: int | None = Field(
...
class ConflictResolutionConfig(BaseModel):
    ...
    config: dict[str, Any] = Field(
        default_factory=dict, description="Additional strategy-specific configuration"
    )
...
class StageErrorHandlingConfig(BaseModel):
    ...
    on_agent_failure: Literal[
        "halt_stage", "retry_agent", "skip_agent", "continue_with_remaining"
...
class StageConfigInner(BaseModel):
    ...
    error_handling: StageErrorHandlingConfig = Field(
        default_factory=StageErrorHandlingConfig
    )
