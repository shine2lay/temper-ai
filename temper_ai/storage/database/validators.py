...
def validate_json_size(
    data: dict[str, Any], max_bytes: int = BYTES_PER_MB, field_name: str = "JSON field"
) -> None:
    """
    ...
    try:
        ...
        logger.debug(f"JSON size validation passed for {field_name}: {size_mb:.2f}MB")

    except JSONSizeError:
        raise
