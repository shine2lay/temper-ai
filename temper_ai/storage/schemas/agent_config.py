...
class SafetyConfig(BaseModel):
    ...
    max_tool_calls_per_execution: int = Field(
        default=MAX_TOOL_CALLS_PER_EXECUTION, gt=0
    )
...
class MemoryConfig(BaseModel):
    ...
    enabled: bool = False
    type: (
        Literal["vector", "episodic", "procedural", "semantic", "cross_session"] | None
    ) = None
    scope: Literal["session", "project", "cross_session", "permanent"] | None = None

    # Memory backend and isolation
    provider: str = Field(
        default="in_memory", description="Memory backend: 'mem0', 'in_memory'"
    )
    memory_namespace: str | None = Field(
        default=None, description="Cross-workflow namespace override"
    )
    tenant_id: str = Field(
        default="default", description="Tenant ID for multi-tenant isolation"
    )
    ...
    def validate_enabled_memory(self) -> "MemoryConfig":
        ...
        if self.enabled and (self.type is None or self.scope is None):
            raise ValueError(
                "When memory is enabled, both type and scope must be specified"
            )
        return self
...
class AgentConfigInner(BaseModel):
    ...
    tools: list[str | ToolReference] | None = None
    ...
    mcp_servers: list[Any] | None = Field(
        default=None,
    ...
    prompt_optimization: Any | None = Field(
        default=None,
    ...
    cross_pollination: Any | None = Field(
        default=None,
    ...
    def validate_cross_pollination(self) -> "AgentConfigInner":
        ...
        if self.cross_pollination is not None and isinstance(
            ...
            self.cross_pollination = CrossPollinationConfig(**self.cross_pollination)
        return self
    ...
    def validate_agent_type_fields(self) -> "AgentConfigInner":
        ...
        if self.type == "script":
            if not self.script:
                raise ValueError("'script' field is required when type='script'")
