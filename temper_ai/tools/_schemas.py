...
class ToolConfigInner(BaseModel):
    ...
    safety_checks: list[str | SafetyCheck] = Field(default_factory=list)
    rate_limits: RateLimits = Field(default_factory=RateLimits)
    error_handling: ToolErrorHandlingConfig = Field(
        default_factory=ToolErrorHandlingConfig
    )
    observability: ToolObservabilityConfig = Field(
        default_factory=ToolObservabilityConfig
    )
