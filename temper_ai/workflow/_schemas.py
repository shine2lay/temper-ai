...
class WorkflowConfigInner(BaseModel):
    ...
    product_type: (
        Literal[
            "web_app", "mobile_app", "api", "data_product", "data_pipeline", "cli_tool"
        ]
        | None
    ) = None
    ...
    observability: WorkflowObservabilityConfig = Field(
        default_factory=WorkflowObservabilityConfig
    )
    ...
    @classmethod
    def validate_stages(
        cls, v: list["WorkflowStageReference"]
    ) -> list["WorkflowStageReference"]:
