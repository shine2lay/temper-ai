...
from temper_ai.workflow.constants import ERROR_MSG_ENV_VAR_PREFIX, MAX_ENV_VAR_SIZE

# Path validation constants
MIN_DRIVE_LETTER_PATH_LENGTH = (
    3  # Minimum length for Windows absolute paths (C:\, D:\, etc.)
)
...
def _check_base_containment(
    ...
    try:
        ...
        return False, (
            f"Path escapes base directory: {path_value} " f"is not within {base_dir}"
        )

    if (base.drive or target.drive) and base.drive != target.drive:
        return False, (f"Path on different drive: {target.drive} vs {base.drive}")
...
class EnvVarValidator:
    ...
    def validate(
        ...
        var_name: str,
        value: str,
        context: ValidationLevel | None = None,
        ...
        is_valid, error = self._check_context_specific(
            var_name, value, validation_level
        )
        if not is_valid:
            return False, error
