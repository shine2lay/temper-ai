...
def agent_outputs_list_strategy(draw, min_size=1, max_size=10):
    ...
    for i in range(num_agents):
        ...
        reasoning = draw(
            st.text(max_size=500, alphabet=st.characters(blacklist_categories=("Cs",)))
        )
        confidence = draw(
            st.floats(
...
class TestConsensusProperties:
    ...
    def test_consensus_returns_valid_decision(self, outputs):
        ...
        assert (
            result.decision in agent_decisions
    ...
    @given(num_agents=st.integers(min_value=1, max_value=10))
    @settings(
