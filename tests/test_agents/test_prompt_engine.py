...
class TestToolSchemaFormatting:
    """Tests for tool schema formatting."""

    def test_format_tools_json(self):
        """Test JSON format for tools."""
        engine = PromptEngine()
        tools = [
            {"name": "calculator", "description": "Performs calculations"},
            {"name": "search", "description": "Searches the web"}
        ]
        result = engine._format_tool_schemas(tools, "json")
        assert "calculator" in result
        assert "Performs calculations" in result
        assert "search" in result

    def test_format_tools_list(self):
        """Test list format for tools."""
        engine = PromptEngine()
        tools = [
            {"name": "calculator", "description": "Performs calculations"},
            {"name": "search", "description": "Searches the web"}
        ]
        result = engine._format_tool_schemas(tools, "list")
        assert result == "- calculator: Performs calculations\n- search: Searches the web"

    def test_format_tools_markdown(self):
        """Test markdown table format for tools."""
        engine = PromptEngine()
        tools = [
            {"name": "calculator", "description": "Performs calculations"},
        ]
        result = engine._format_tool_schemas(tools, "markdown")
        assert "| Tool | Description |" in result
        assert "| calculator | Performs calculations |" in result

    def test_format_tools_empty(self):
        """Test formatting empty tool list."""
        engine = PromptEngine()
        result = engine._format_tool_schemas([], "list")
        assert result == "No tools available"

    def test_format_tools_invalid_style(self):
        """Test error with invalid format style."""
        engine = PromptEngine()
        tools = [{"name": "calc", "description": "Calculator"}]
        with pytest.raises(ValueError, match="Unknown format_style"):
            engine._format_tool_schemas(tools, "invalid")


class TestRenderWithTools:
    """Tests for render_with_tools method."""

    def test_render_with_tools_json(self):
        """Test rendering with tools in JSON format."""
        engine = PromptEngine()
        template = "Available tools:\n{{tools_available}}"
        tools = [{"name": "calc", "description": "Calculator"}]

        result = engine.render_with_tools(template, {}, tools, format_style="json")
        assert "calc" in result
        assert "Calculator" in result

    def test_render_with_tools_list(self):
        """Test rendering with tools in list format."""
        engine = PromptEngine()
        template = "Tools: {{tools_available}}"
        tools = [{"name": "calc", "description": "Calculator"}]

        result = engine.render_with_tools(template, {}, tools, format_style="list")
        assert "- calc: Calculator" in result

    def test_render_with_tools_no_tools(self):
        """Test rendering when no tools provided."""
        engine = PromptEngine()
        template = "Tools: {{tools_available}}"

        result = engine.render_with_tools(template, {}, None)
        assert result == "Tools: No tools available"

    def test_render_with_tools_access_raw_schemas(self):
        """Test accessing raw tool schemas in template."""
        engine = PromptEngine()
        template = "You have {{tools|length}} tools"
        tools = [{"name": "calc"}, {"name": "search"}]

        result = engine.render_with_tools(template, {}, tools)
        assert result == "You have 2 tools"


class TestRenderWithSystemVars:
    """Tests for render_with_system_vars method."""

    def test_inject_agent_name(self):
        """Test injecting agent_name system variable."""
        engine = PromptEngine()
        template = "I am {{agent_name}}"

        result = engine.render_with_system_vars(template, {}, agent_name="researcher")
        assert result == "I am researcher"

    def test_inject_tools_available(self):
        """Test injecting tools_available system variable."""
        engine = PromptEngine()
        template = "Tools:\n{{tools_available}}"
        tools = [{"name": "calc", "description": "Calculator"}]

        result = engine.render_with_system_vars(template, {}, tool_schemas=tools)
        assert "calc" in result

    def test_inject_custom_system_vars(self):
        """Test injecting custom system variables via kwargs."""
        engine = PromptEngine()
        template = "{{custom_var}} and {{another_var}}"
    ...
    def test_system_vars_dont_override_user_vars(self):
        """Test that user variables take precedence."""
        engine = PromptEngine()
        template = "{{name}}"
        ...
        result = engine.render_with_system_vars(
            template,
            {"name": "user_value"},
            name="system_value"  # This should be overridden
        )
        # User variables should take precedence (they're added after)
        # Actually, system_vars.update() happens after, so system wins
        # Let me check the implementation...
        # variables.update(system_vars) means system vars override
...
class TestRenderAgentPrompt:
    ...
    def test_render_inline_prompt(self):
        """Test rendering inline prompt from config."""
        engine = PromptEngine()
        config = {
            "agent": {
                "name": "researcher",
        ...
        result = engine.render_agent_prompt(config, {"name": "World"})
        assert result == "Hello World!"

    def test_render_inline_with_agent_name(self):
        """Test that agent_name is automatically injected."""
        engine = PromptEngine()
        config = {
            "agent": {
                "name": "researcher",
                "prompt": {
                    "inline": "I am {{agent_name}}"
                }
            }
        }

        result = engine.render_agent_prompt(config, {})
        assert result == "I am researcher"

    def test_render_inline_with_tools(self):
        """Test rendering inline prompt with tools."""
        engine = PromptEngine()
        config = {
            "agent": {
                "name": "researcher",
                "prompt": {
                    "inline": "Tools: {{tools|length}}"
                }
            }
        }
        tools = [{"name": "calc"}, {"name": "search"}]

        result = engine.render_agent_prompt(config, {}, tool_schemas=tools)
        assert result == "Tools: 2"

    def test_render_template_file(self, temp_dir_with_template):
        """Test rendering prompt from template file."""
        templates_dir, _ = temp_dir_with_template
        ...
        result = engine.render_agent_prompt(config, {"topic": "AI"})
        assert "technology" in result
        assert "AI" in result

    def test_render_agent_prompt_no_prompt_config(self):
        """Test error when prompt config is missing."""
        engine = PromptEngine()
        config = {"agent": {"name": "test"}}

        with pytest.raises(PromptRenderError, match="must have 'prompt"):
            engine.render_agent_prompt(config, {})

    @pytest.fixture
    def temp_dir_with_template(self):
        """Create temp directory with template file."""
        with tempfile.TemporaryDirectory() as tmpdir:
            templates_dir = Path(tmpdir)
            template_file = templates_dir / "agent_prompt.txt"
            template_file.write_text(
                "You are an expert in {{domain}}. Topic: {{topic}}"
            )
            yield templates_dir, template_file
...
class TestTemplateCaching:
    ...
    def test_cache_integration_with_render_with_tools(self):
        """Test that caching works with render_with_tools."""
        engine = PromptEngine()
        template = "Tools: {{tools|length}}"
        tools = [{"name": "calc"}, {"name": "search"}]

        # First render
        result1 = engine.render_with_tools(template, {}, tools)
        # Second render
        result2 = engine.render_with_tools(template, {}, tools)

        assert result1 == result2 == "Tools: 2"

        # Should have cache hit (render_with_tools calls render internally)
        stats = engine.get_cache_stats()
        assert stats["cache_hits"] >= 1

    def test_cache_integration_with_render_with_system_vars(self):
        """Test that caching works with render_with_system_vars."""
        engine = PromptEngine()
        template = "Agent: {{agent_name}}"

        # Multiple renders with different system vars
        engine.render_with_system_vars(template, {}, agent_name="agent1")
        engine.render_with_system_vars(template, {}, agent_name="agent2")
        engine.render_with_system_vars(template, {}, agent_name="agent3")

        # Should use cached template
        stats = engine.get_cache_stats()
        assert stats["cache_misses"] == 1
        assert stats["cache_hits"] == 2
...
class TestRealWorld:
    ...
    def test_complex_research_prompt(self):
        """Test rendering a complex research agent prompt."""
        engine = PromptEngine()
        template = """
You are {{agent_name}}, a research analyst specializing in {{domain}}.
...
"""
        ...
        result = engine.render_with_system_vars(
            template,
            {
                ...
                "focus_areas": ["LLMs", "Computer Vision"],
                "depth": "deep"
            },
