...
async def test_e2e_concurrent_workflows_throughput():
    ...
    results = await asyncio.gather(*[execute_workflow(i) for i in range(num_workflows)])
    ...
    assert (
        throughput >= 2.0
    ), f"Throughput {throughput:.2f} workflows/sec below target 2.0"
