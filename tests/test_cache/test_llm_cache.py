...
class TestRedisCacheInitialization:
    ...
    def test_init_requires_redis_package(self):
        ...
        with patch("src.cache.llm_cache.REDIS_AVAILABLE", False):
    ...
    def test_init_with_password_env_var(self):
        ...
        with patch.dict(os.environ, {"REDIS_PASSWORD": "secret123"}):
            with patch("src.cache.llm_cache.REDIS_AVAILABLE", True):
                with patch("src.cache.llm_cache.redis") as mock_redis_module:
                    mock_client = Mock()
...
class TestRedisCacheOperations:
    ...
    def test_set_and_get(self):
        """Test basic set and get operations."""
        with patch("src.cache.llm_cache.redis.Redis") as mock_redis:
            mock_client = Mock()
