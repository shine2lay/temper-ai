...
class TestMetricAggregatorInitialization:
    ...
    def test_init_with_backend_and_registry(self):
        ...
        aggregator = MetricAggregator(
            backend=mock_backend, metric_registry=mock_registry
        )
...
class TestSetAgentOutput:
    ...
    def test_set_agent_output_minimal(self):
        ...
        aggregator.set_agent_output(agent_id="agent-123", output_data=output_data)

        mock_backend.set_agent_output.assert_called_once()
        call_kwargs = mock_backend.set_agent_output.call_args[1]
        assert call_kwargs["agent_id"] == "agent-123"
        assert call_kwargs["output_data"] == output_data
        metrics = call_kwargs["metrics"]
        assert isinstance(metrics, AgentOutputData)
        assert metrics.reasoning is None
    ...
    def test_set_agent_output_delegates_to_backend(self):
        ...
        aggregator.set_agent_output(
            agent_id="agent-789", output_data={"status": "complete"}
        )
...
class TestSetStageOutput:
    ...
    def test_set_stage_output(self):
        ...
        aggregator.set_stage_output(stage_id="stage-123", output_data=output_data)
    ...
    def test_set_stage_output_empty_data(self):
        ...
        aggregator.set_stage_output(stage_id="stage-456", output_data={})
...
class TestBackendDelegation:
    ...
    def test_all_methods_delegate_to_backend(self):
        ...
        # Call all methods
        aggregator.set_agent_output(agent_id="agent-1", output_data={"data": "test"})
    ...
    def test_backend_method_signatures_preserved(self):
        ...
        # Verify parameters were passed (metrics bundled in AgentOutputData)
        call_kwargs = mock_backend.set_agent_output.call_args[1]
        assert call_kwargs["agent_id"] == "agent-test"
        assert call_kwargs["metrics"].reasoning == "test reasoning"
        assert call_kwargs["metrics"].confidence_score == 0.8
        assert call_kwargs["metrics"].total_tokens == 100
...
class TestErrorResilience:
    ...
    def test_backend_error_in_set_agent_output(self):
        ...
        with pytest.raises(Exception, match="Backend error"):
            aggregator.set_agent_output(
                agent_id="agent-1", output_data={"data": "test"}
            )
...
class TestEdgeCases:
    ...
    def test_empty_output_data(self):
        ...
        aggregator.set_agent_output(agent_id="agent-empty", output_data={})
    ...
    def test_very_large_output_data(self):
        ...
        aggregator.set_agent_output(agent_id="agent-large", output_data=large_data)
    ...
    def test_negative_token_counts(self):
        ...
        aggregator.set_agent_output(
            agent_id="agent-negative", output_data={"test": "data"}, total_tokens=-100
        )
    ...
    def test_confidence_score_out_of_range(self):
        ...
        # Aggregator doesn't validate, just passes to backend
        aggregator.set_agent_output(
            agent_id="agent-conf", output_data={"test": "data"}, confidence_score=1.5
        )

        call_kwargs = mock_backend.set_agent_output.call_args[1]
        assert call_kwargs["metrics"].confidence_score == 1.5
    ...
    def test_none_agent_id(self):
        ...
        aggregator.set_agent_output(agent_id=None, output_data={"test": "data"})
