...
class TestGetPeriodStart:
    ...
    def test_get_period_start_minute(self):
        ...
        expected = datetime(2024, 1, 1, 14, 29, 0, tzinfo=UTC)
    ...
    def test_get_period_start_hour(self):
        ...
        expected = datetime(2024, 1, 1, 13, 30, 0, tzinfo=UTC)
        assert start_time == expected

    def test_get_period_start_day(self):
        """Test period start calculation for DAY."""
        end_time = datetime(2024, 1, 5, 14, 30, 0, tzinfo=UTC)
        start_time = TimeWindowCalculator.get_period_start(
            end_time, AggregationPeriod.DAY
        )

        expected = datetime(2024, 1, 4, 14, 30, 0, tzinfo=UTC)
        assert start_time == expected
    ...
    def test_get_period_start_minute_crosses_hour(self):
        ...
        expected = datetime(2024, 1, 1, 13, 59, 0, tzinfo=UTC)
        assert start_time == expected

    def test_get_period_start_hour_crosses_day(self):
        """Test HOUR period that crosses day boundary."""
        end_time = datetime(2024, 1, 2, 0, 30, 0, tzinfo=UTC)
        start_time = TimeWindowCalculator.get_period_start(
            end_time, AggregationPeriod.HOUR
        )

        expected = datetime(2024, 1, 1, 23, 30, 0, tzinfo=UTC)
        assert start_time == expected

    def test_get_period_start_day_crosses_month(self):
        """Test DAY period that crosses month boundary."""
        end_time = datetime(2024, 2, 1, 14, 30, 0, tzinfo=UTC)
        start_time = TimeWindowCalculator.get_period_start(
            end_time, AggregationPeriod.DAY
        )

        expected = datetime(2024, 1, 31, 14, 30, 0, tzinfo=UTC)
        assert start_time == expected

    def test_get_period_start_preserves_microseconds(self):
        """Test that microseconds are preserved."""
        end_time = datetime(2024, 1, 1, 14, 30, 0, 123456, tzinfo=UTC)
        start_time = TimeWindowCalculator.get_period_start(
            end_time, AggregationPeriod.MINUTE
        )
...
class TestGetDefaultTimeWindow:
    ...
    def test_get_default_time_window_minute(self):
        ...
        start_time, returned_end = TimeWindowCalculator.get_default_time_window(
            AggregationPeriod.MINUTE, end_time=end_time
        )

        expected_start = datetime(2024, 1, 1, 14, 29, 0, tzinfo=UTC)
    ...
    def test_get_default_time_window_hour(self):
        ...
        end_time = datetime(2024, 1, 1, 14, 30, 0, tzinfo=UTC)
        start_time, returned_end = TimeWindowCalculator.get_default_time_window(
            AggregationPeriod.HOUR, end_time=end_time
        )

        expected_start = datetime(2024, 1, 1, 13, 30, 0, tzinfo=UTC)
        assert start_time == expected_start
        assert returned_end == end_time
    ...
    def test_get_default_time_window_day(self):
        ...
        end_time = datetime(2024, 1, 5, 14, 30, 0, tzinfo=UTC)
        start_time, returned_end = TimeWindowCalculator.get_default_time_window(
            AggregationPeriod.DAY, end_time=end_time
        )

        expected_start = datetime(2024, 1, 4, 14, 30, 0, tzinfo=UTC)
        assert start_time == expected_start
        assert returned_end == end_time
...
class TestTimeWindowCalculatorEdgeCases:
    ...
    def test_leap_year_day_period(self):
        ...
        end_time = datetime(2024, 3, 1, 12, 0, 0, tzinfo=UTC)
        start_time = TimeWindowCalculator.get_period_start(
            end_time, AggregationPeriod.DAY
        )

        # 2024 is a leap year, so Feb 29 exists
        expected = datetime(2024, 2, 29, 12, 0, 0, tzinfo=UTC)
        assert start_time == expected

    def test_year_boundary(self):
        """Test period that crosses year boundary."""
        end_time = datetime(2024, 1, 1, 0, 30, 0, tzinfo=UTC)
        start_time = TimeWindowCalculator.get_period_start(
            end_time, AggregationPeriod.HOUR
        )

        expected = datetime(2023, 12, 31, 23, 30, 0, tzinfo=UTC)
        assert start_time == expected
