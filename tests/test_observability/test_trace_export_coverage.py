...
class TestFlattenForWaterfall:
    """Test flatten_for_waterfall."""
    ...
    def test_flatten_basic(self):
        ...
        assert len(flat) == 2
        assert flat[0]["id"] == "wf-1"
        assert flat[0]["depth"] == 0
        assert flat[1]["id"] == "stage-1"
        assert flat[1]["depth"] == 1
        assert flat[1]["start_offset_ms"] == 1000

    def test_flatten_with_none_duration(self):
        trace = {
            "id": "wf-1",
            "name": "workflow",
            "type": "workflow",
    ...
    def test_flatten_deep_hierarchy(self):
        ...
        flat = flatten_for_waterfall(trace)
        assert len(flat) == 4
        depths = [item["depth"] for item in flat]
