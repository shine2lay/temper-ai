...
class TestBlastRadiusPolicyValidation:
    ...
    def test_excessive_max_files_rejected(self):
        ...
        with pytest.raises(ValueError, match="max_files_per_operation must be <= 10000"):
    ...
    def test_non_numeric_max_files_rejected(self):
        ...
        with pytest.raises(ValueError, match="max_files_per_operation must be a number"):
    ...
    def test_excessive_ops_per_minute_rejected(self):
        ...
        with pytest.raises(ValueError, match="max_operations_per_minute must be <= 1000"):
    ...
    def test_forbidden_patterns_non_string_item_rejected(self):
        ...
        with pytest.raises(ValueError, match="forbidden_patterns\\[0\\] must be a string"):
    ...
    def test_forbidden_patterns_oversized_pattern_rejected(self):
        ...
        huge_pattern = "x" * 1001
        with pytest.raises(ValueError, match="forbidden_patterns\\[\\d+\\] exceeds maximum length"):
...
class TestPolicySecurityValidation:
    ...
    @pytest.mark.parametrize("malicious_value", [
        -1, -999999, 0, float('inf'), float('-inf')
    ])
