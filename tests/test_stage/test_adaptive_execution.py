...
class TestDisagreementRateCalculation:
    ...
    def test_calculate_disagreement_unanimous(self):
        ...
        disagreement_rate = compiler.executors["adaptive"]._calculate_disagreement_rate(
            synthesis_result
        )
...
class TestAdaptiveExecution:
            ...
            with patch(
                ...
                with patch(
                    ...
                    with patch(
                        ...
                        assert (
                            stage_output["mode_switch"]["switched_to"] is None
                        )  # No switch
            ...
            with patch(
                ...
                with patch(
                    ...
                    with patch(
                        ...
                        assert stage_output["mode_switch"]["switched_to"] in (
                            None,
                            "sequential",
                        )
            ...
            with patch(
                ...
                with patch(
                    ...
                    with patch(
                        ...
                        assert (
                            "mode_switch" in stage_output
                        ), "Expected mode_switch metadata in stage output"
                        assert (
                            stage_output["mode_switch"]["disagreement_threshold"] == 0.5
                        )
    ...
    def test_adaptive_tracks_mode_switch_in_observability(self, mock_agents):
        ...
        state = {"workflow_id": "wf-123", "stage_outputs": {}, "tracker": mock_tracker}
...
class TestAdaptiveExecutionEdgeCases:
    ...
    def test_adaptive_handles_parallel_failure(self):
        ...
        with patch.object(adaptive_executor, "parallel_executor") as mock_parallel_exec:
            with patch.object(
                adaptive_executor, "sequential_executor"
            ) as mock_sequential_exec:
                mock_parallel_exec.execute_stage.side_effect = RuntimeError(
                ...
                assert (
                    result["stage_outputs"]["research"]["mode_switch"]["switched_to"]
                    == "sequential"
                )
