...
class TestAtomicWritePattern:
    ...
    def test_uses_os_replace(self, backend, domain_state):
        ...
        with patch("src.workflow.checkpoint_backends.os.replace", wraps=os.replace) as mock_replace:
    ...
    def test_uses_tempfile(self, backend, domain_state):
        ...
        with patch("src.workflow.checkpoint_backends.tempfile.mkstemp", wraps=tempfile.mkstemp) as mock_mkstemp:
            cp_id = backend.save_checkpoint("wf-lock-test", domain_state)
...
class TestCrashRecovery:
    ...
    def test_previous_checkpoint_intact_on_write_failure(self, backend, domain_state):
        ...
        with patch("src.workflow.checkpoint_backends.json.dump", side_effect=crashing_dump):
    ...
    def test_temp_file_cleaned_on_failure(self, backend, temp_dir):
        ...
        with patch("src.workflow.checkpoint_backends.json.dump", side_effect=IOError("disk full")):
            with pytest.raises(IOError):
...
class TestFsyncCalled:
    ...
    def test_fsync_called_before_replace(self, backend, domain_state):
        ...
        with patch("src.workflow.checkpoint_backends.os.fsync", side_effect=tracking_fsync):
            with patch("src.workflow.checkpoint_backends.os.replace", side_effect=tracking_replace):
                backend.save_checkpoint("wf-lock-test", domain_state)
